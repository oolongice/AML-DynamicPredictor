...
#%%
# HGB 
parameters = "../FigureS2_ODE_fitting/optimized_parameters_and_R2_WBC.csv"
parameters = pd.read_csv(parameters)
results_list = []
def WBC_ode_system(x, t, p, get_u, dosage_timeline):
...
for patient_id in patient_ids:
    ...
    solution = solve_ivp(
        lambda t, x: WBC_ode_system(x, t, p, get_u, dosage_timeline), 
        (t_start, t_end), 
        x0_updated, 
        method='RK45'
    )
...
WBC_error = pd.DataFrame(results_list)
save_path = "../data/all_wbc_nrmse.csv"
WBC_error.to_csv(save_path, index=False)